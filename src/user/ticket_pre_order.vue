<template>
  <div class="detail-page">
    <div class="weui-cells__title page-title">班车团购信息</div>
    <div class="weui-cells">
      ...
      <div class="weui-cell">
        ...
        <div class="weui-cell__ft">{{formatDate(dispatch.pre_order_end_time)}}</div>
      </div>
      <div class="weui-cell">
        <div class="weui-cell__bd">
          <p>截止时间</p>
        </div>
        <div class="weui-cell__ft">{{formatTime(dispatch.pre_order_end_time)}}</div>
      </div>
      <!--<div class="weui-cell">
        <div class="weui-cell__bd">
          <p>停止发售</p>
    ...
    <div class="btn-wrapper"><a href="javascript:void(0);" @click="newPreOrder" class="weui-btn weui-btn_primary" :class="{'weui-btn_disabled': noOrder}">预订</a></div>
  ...
  export default {
    ...
    computed:{
      dueDate:function(){
        if(!this.detail.departure)
          return false
        var d=new Date(this.detail.departure.getTime())
      ...
      'noOrder': function () {
        ...
        return ((!this.detail.leftover || this.detail.stopOrder))
      },
    ...
    methods: {
      formatTime: function (dateObj) {
        return util.formatTime(new Date(dateObj))
      },
      ...
      newPreOrder () {
        ...
        vm.$http.get(`/api/users/${user.id}`).then((res) => {
          ...
          var content = `您当前账户余额￥${user.balance}元，确定预订？`
          if (vm.detail.price > user.balance) {
            content = `您当前账户余额￥${user.balance}元，还需支付￥${Number(vm.detail.price - user.balance).toFixed(2)}元，确定预订？`
          }
          ...
          vm.showDialog({
            ...
            primary: {
              ...
              click: function () {
                ...
                buying = true
                vm.$http.post(`/api/users/${user.id}/orders/preOrder`, {dispatch: vm.dispatch}).then((res) => {
    ...
    ready:function(){
      ...
      vm.$http.get('/api/dispatches/' + dispatchId).then(function(res){
        ...
        var leftover= dispatch.use_full_price?(dispatch.full_price_seat_count>dispatch.full_price_sold_count) : (dispatch.seat_count>dispatch.sold_count)
